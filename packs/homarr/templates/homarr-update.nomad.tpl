...
echo "Fetching latest Homarr container version..."

apt-get update -qq && apt-get install -y -qq curl jq unzip > /dev/null 2>&1

# Fetch latest stable tag from GHCR via GitHub API
TAG=$(curl -s "https://api.github.com/repos/homarr-labs/homarr/releases/latest" | \
    jq -r '.tag_name' | sed 's/^v//')

if [ -z "$TAG" ] || [ "$TAG" = "null" ]; then
    echo "Error: Failed to find latest Homarr release tag"
    exit 1
fi

echo "Latest homarr tag: $TAG"
