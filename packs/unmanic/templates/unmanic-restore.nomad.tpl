...
CONFIG_DIR="/unmanic-config/.unmanic/config"
PLUGINS_DIR="/unmanic-config/.unmanic/plugins"
...
if [ -f "$RESTORE_SOURCE/unmanic.db" ]; then
    echo "Restoring Unmanic database..."
    cp "$RESTORE_SOURCE/unmanic.db" "$CONFIG_DIR/"
    echo "Database restore complete."
else
    echo "Warning: No unmanic.db found in backup"
fi
...
if [ -d "$RESTORE_SOURCE/plugins" ]; then
    echo "Restoring Unmanic plugins..."
    rsync -av "$RESTORE_SOURCE/plugins/" "$PLUGINS_DIR/"
...
chown -R [[ var "unmanic_uid" . ]]:[[ var "unmanic_gid" . ]] "/unmanic-config/.unmanic" 2>/dev/null || echo "Warning: Could not set ownership"
